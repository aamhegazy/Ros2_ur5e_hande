...
def generate_launch_description():
    ...
    declared_args.append(DeclareLaunchArgument(
        ...
        description="IP for the Unity TCP endpoint to bind on"
    ))
    ...
    # --- 3. MOVE GROUP ---
    # MoveGroup has its own internal MultiThreadedExecutor.
    # MOVEIT_NUM_MOVE_GROUP_THREADS controls how many threads it spins.
    move_group_node = Node(
        ...
        additional_env={"MOVEIT_NUM_MOVE_GROUP_THREADS": "4"},
    )
    ...
    # ros_tcp_endpoint already runs its TCP listener on a background thread.
    # executor_type=MultiThreadedExecutor ensures ROS pub/sub callbacks
    # created dynamically per Unity topic don't block each other or the driver.
    unity_tcp_endpoint = Node(
        ...
        additional_env={
            "ROS_TCP_ENDPOINT_NUM_THREADS": "4",
        },
    )
    ...
    return LaunchDescription(declared_args + [
        ...
        control_node,
        unity_tcp_endpoint,          # starts alongside driver, not blocked by controllers
        spawn_jsb,
